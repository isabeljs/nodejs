...
describe("response", () => {
  ...
  const _checkResponseStatus = (statusName, status) => {
  ...
  it("should provide aliases for common success statuses", () => {
  ...
  it("should provide aliases for common client errors statuses", () => {
  ...
  it("should provide aliases for common server errors statuses", () => {
  ...
  it("should handle collections of resources", () => {
    const resources = [
      { value: "v1" },
      { value: "v2" }
    ]
  ...
  it("should fail when trying to iterate a single resource", () => {
    const created = new response.Response(200, { foo: "bar" })
    let iterated = false
    try {
      created.forEach(() => {}) // eslint-disable-line no-empty-function
      iterated = true
    } catch (error) {
      error.should.be.an.instanceOf(Error)
      error.message.should.equal("Response does not contain iterable resources")
    } finally {
  ...
  it("should handle links", () => {
    const created = new response.Resource({ foo: "bar" })
