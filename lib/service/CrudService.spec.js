...
describe("CrudService", () => {
  ...
  it("should not be instantiable", () => {
    ...
    try {
      new CrudService() // eslint-disable-line no-new
      instantiated = true
    } catch (error) {
  ...
  it("should not accept an argument that is not an instance of CrudRepository", () => {
    let instantiated = false
    try {
      new class extends CrudService {} // eslint-disable-line no-new
      instantiated = true
    } catch (error) {
      error.should.be.an.instanceOf(TypeError)
  ...
  it("should delegate any method call to enclosed repository", function *() {
    ...
    for (const method of EXPECTED_METHODS) {
      ...
      spy = spy.withArgs(...EXPECTED_METHODS_PARAMETERS[method])
      const result = yield myService[method](...EXPECTED_METHODS_PARAMETERS[method])
