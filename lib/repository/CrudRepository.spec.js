...
describe("CrudRepository", () => {
  ...
  it("should force anonymous subclasses to implement expected CRUD methods", function *() {
    const myRepository = new class extends CrudRepository {}
    for (const method of ACTUAL_METHODS) {
      let called = false
      try {
        yield myRepository[method]()
        ...
        error.should.be.an.instanceOf(TypeError)
        error.message.should.equal(`CrudRepository.${method}(...) needs to be overridden by anonymous subclass`)
      } finally {
        called.should.be.false()
      }
    }
