...
describe("MongoCrudRepository", () => {
  ...
  function *_assertArticlesCollectionContainsOnly(article) {
    const articles = yield _articlesCollection.find().toArray()
    articles.should.eql([article])
  }

  function *_assertArticlesCollectionIsEmpty() {
    const articles = yield _articlesCollection.find().toArray()
    articles.should.eql([])
  }

  function *_checkInsertWhenReturningDocument(...commands) {
  ...
  function *_checkReplaceWhenReturningDocument(...commands) {
    for (const command of commands) {
  ...
  function *_checkUpdateWhenReturningDocument(...commands) {
    for (const command of commands) {
      const article = yield _createArticle()
      const update = {
        title: "Updated article title",
  ...
  function *_checkDeleteWhenReturningDocument(...commands) {
    for (const command of commands) {
      const article = yield _createArticle()
  ...
  function *_checkDeleteWhenReturningDocuments(...commands) {
    for (const command of commands) {
  ...
  before(function *() {
    _database = yield MongoClient.connect("mongodb://localhost:27017/isabel")
  ...
  it("should not be instantiable", () => {
    ...
    try {
      new MongoCrudRepository() // eslint-disable-line no-new
      instantiated = true
    } catch (error) {
  ...
  it("should find documents", function *() {
    const article1 = yield _createArticle()
    const article2 = yield _createArticle()
    const found = yield _articleRepository.findMany()
